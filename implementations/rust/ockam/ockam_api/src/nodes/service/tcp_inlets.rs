...
use ockam::udp::UdpHolePuncherNegotiation;
use ockam::Result;
...
impl InletSessionReplacer {
    ...
    async fn spawn_udp_puncture(
        ...
        inlet: Arc<TcpInlet>, // TODO: Replace with a RwLock
        disable_tcp_fallback: bool,
    ) -> Result<()> {
        // self.node_manager.create_udp_puncture()

        let mut main_route = connection.route()?;
        // FIXME: Trim outlet part
        let main_route: Route = main_route.modify().pop_back().into();
        let sc_side_route = route![main_route.clone(), DefaultAddress::SECURE_CHANNEL_LISTENER]; // FIXME: Don't assume listener address here

        // TODO: Make it as a part of Replacer's state
        let sc_side = self
            .node_manager
            .create_secure_channel_internal(
                ...
                None,
                Some(self.wait_for_outlet_duration), // FIXME: Is that the right timeout here?
        ...
        let (mut receiver, sender) = UdpHolePuncherNegotiation::start_negotiation(
            &self.context,
            route![
                main_route.clone(),
                DefaultAddress::UDP_HOLE_PUNCHER_LISTENER
            ],
            &self.node_manager.udp_transport,
            rendezvous_route,
        )
        .await?;
            ...
            loop {
                match receiver.recv().await {
                    Ok(route) => {
                        ...
                        // TODO: Add more meaningful return type from receiver
                        if !route.is_empty() {
                            info!("Updating route to UDP: {}", route);

                            // TODO: Monitor this side as well? Also monitored on the UDP puncture level
                            let _res = sc_side.update_remote_node_route(route.clone()); // TODO: Handle error
