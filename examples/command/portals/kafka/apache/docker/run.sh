...
# https://docs.ockam.io/portals/kafka/apache/docker
...
run() {
  ...
  #
  # The identity will also allowed to create a relay in the project at the address `kafka`.
  kafka_operator_ticket=$(ockam project ticket --usage-count 1 --expires-in 10m --relay kafka)
  ...
  # Invoke `docker-compose up` in the directory that has kafka_operator's configuration.
  # Pass the above enrollment ticket as an environment variable.
  #
  # Read kafka_operator/docker-compose.yml to understand the parts that are provisioned
  # in kafka_operator's virtual private network.
  echo; pushd kafka_operator; ENROLLMENT_TICKET="$kafka_operator_ticket" docker compose up -d; popd
...
cleanup() {
  pushd kafka_operator; docker compose down --rmi all --remove-orphans; popd
