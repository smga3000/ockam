...
use ockam::udp::{UdpHolePuncherNegotiationListener, UdpTransportExtension, UDP};
use ockam::Context;
use ockam::{node, route, Node, Result};
...
async fn do_main(node: &mut Node) -> Result<()> {
    ...
    UdpHolePuncherNegotiationListener::create(node.context(), "udp", &udp, rendezvous_route).await?;
